...
class kmscm(nn.Module):
    def __init__(self,in_channel,out_channel,k1,k2):
        ...
        self.conv1=nn.Conv2d(in_channel//2,k1,kernel_size=1,stride=1)
    ...
    def forward(self,x:torch.tensor):
        ...
        for c in range(B):
            label_s=[]
            {label_s.append(self.kmeans2.fit(np.nan_to_num(preprocessing.minmax_scale(v22_c[c,kk,:,:].view(-1,1).detach().cpu().numpy(),axis=0), nan=0.0, posinf=0.0, neginf=0.0)).labels_) for kk in range(v22_c.shape[1])}
            #采用形态学方法重构图像
            eroded=[]
            rec=[]
            fin=[]
        ...
        #重新使用pytorch
        #unsqueeze(),向前增加一个维度
        fin_attention=self.sigmoid(fin_attention)
