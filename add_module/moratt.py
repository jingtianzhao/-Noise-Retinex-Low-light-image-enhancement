...
class moratt(torch.nn.Module):
    ...
    def __init__(self,channel1,channel2,s2):
        super().__init__()
        #通道注意力
        ...
        # self.size2=torch.ones(s2,s2)
        self.se_logits = nn.Parameter(torch.zeros(s2, s2))
        # self.input=None
        # self.output=None
        # self.A=[]
        # self.B=[]
    ...
    def forward(self, x):
        ...
        # self.A=x_spatial
        se = torch.sigmoid(self.se_logits)
        x_spatial=self.morp(x_spatial,se).view(b,1,-1,1).softmax(2)
        # self.C=x_spatial
        # x_spatial=self.morp(x_spatial,self.size2.to(device)).view(b,1,-1,1).softmax(2)
